  ...
  const { data: productRevenue, isLoading: isRevenueLoading } = useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      const res = await fetch(
        `/api/analytics/product-revenue?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}`
      )
            ...
            <CardContent>
              {isRevenueLoading ? (
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={productRevenue?.slice(0, 10)} margin={{ top: 20, right: 30, left: 40, bottom: 70 }}>
        ...
        <TabsContent value="conversion">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Conversion rate per category</CardDescription>
            </CardHeader>
            <CardContent>
              {isDashboardLoading ? (
                <div className="h-80 flex items-center justify-center">Loading category data...</div>
    ...
    <Card>
      <CardContent className="p-6">
        <div className="flex items-center">
          <div className="flex-shrink-0 bg-primary rounded-md p-3">{getIcon()}</div>
